...
from ..helpers.sql_helper.bot_blacklists import add_user_to_bl, rem_user_from_bl
from ..helpers.sql_helper.bot_pms_sql import get_user_id
...
async def ban_user_from_bot(user, reason, reply_to=None):
    ...
    await techno.tgbot.send_message(user.id, banned_msg)
    info = f"**#Banned_Bot_PM_User**\
            ...
            \n**Reason:** `{reason}`"
    if BOTLOG:
        await techno.send_message(BOTLOG_CHATID, info)
    return info
