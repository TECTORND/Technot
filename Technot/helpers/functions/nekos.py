...
async def fakegs(search, result):
    ...
    font1 = ImageFont.truetype(
        "Technot/helpers/styles/ProductSans-BoldItalic.ttf", 20
    )
    font2 = ImageFont.truetype("Technot/helpers/styles/ProductSans-Light.ttf", 23)
...
async def trumptweet(text):
    ...
    TECHNO = r.get("message")
    technourl = url(TECHNO)
    if not technourl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(TECHNO).content)
