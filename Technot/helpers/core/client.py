...
class TechnoClient(TelegramClient):
    def techno_cmd(
        self: TelegramClient,
        ...
        def decorator(func):  # sourcery no-metrics
            async def wrapper(check):  # sourcery no-metrics
                ...
                if hasattr(chat, "title"):
                    if ("#noub" in chat.title.lower() and not (chat.admin_rights or chat.creator)):
                      return
                ...
                try:
                    ...
                    if not disable_errors:
                        ...
                        text = "**Technot Error report**\n\n"
                        link = "[here](https://t.me/Technot_Support)"
            ...
            if pattern is not None:
                ...
                if allow_sudo and getgvar("sudoenable") is not None:
                ...
                if edited:
                    techno.add_event_handler(func, events.MessageEdited(**kwargs))
                techno.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            ...
            if edited is True:
                techno.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                techno.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
...
TechnoClient.fast_download_file = download_file
TechnoClient.fast_upload_file = upload_file
TechnoClient.reload = restart_script
TechnoClient.get_msg_link = get_message_link
TechnoClient.check_testcases = checking
try:
    send_message_check = TelegramClient.send_message
except AttributeError:
    TechnoClient.send_message = send_message
    TechnoClient.send_file = send_file
    TechnoClient.edit_message = edit_message