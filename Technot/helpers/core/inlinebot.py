...
@techno.tgbot.on(CallbackQuery(data=re.compile(b"help_k_minu")))
@check_owner
...
def main_menu():
    ...
    buttons = [
        ...
        [
            ...
            Button.url("Support 🇮🇳", "https://t.me/Technot_Updates"),
        ],
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**⚜ Technot"):
            ...
            result = builder.article(
                ...
                buttons=[
                    ...
                    [
                        Button.url(
                            "💞 Deploy 💞",
                            "https://heroku.com/deploy?template=https://github.com/TeamTechnot/Technot",
                        )
                    ],
                ],
            )
            ...
            buttons = []
            media = None
            technomedia = MEDIA_PATH_REGEX.search(markdown_note)
            if technomedia:
                media = technomedia.group(2)
                markdown_note = markdown_note.replace(technomedia.group(0), "")
            ...
            builder = event.builder
            troll = os.path.join("./Technot", "troll.txt")
            try:
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        ...
                        TECHNO = f"[{u.first_name}](tg://user?id={u.id})"
                    u = int(u.id)
                except ValueError:
                    # ValueError: Could not find the input entity
                    TECHNO = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                title="Troll Message",
                text=f"🌹 Only This : {TECHNO} cannot access this message !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./Technot", "secrets.txt")
            try:
            ...
            result = builder.article(
                ...
                text=f"🔒 A whisper message to {TECHNO}, Only he/she can open it.",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./Technot", "hide.txt")
            try:
            ...
            if oso == "OFF":
                result = builder.article(
                    title="© Technot Help",
                    description="Help menu for Technot",
                    text=_result[0],
                    buttons=_result[1],
                    link_preview=False,
                )
            ...
            buttons = [
                (
                    Button.url("Source code", "https://github.com/TeamTechnot/Technot"),
                    Button.url(
                        "Deploy",
                        "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FTeamTechnot%2FTechnot&template=https%3A%2F%2Fgithub.com%2FTeamTechnot%2FTechnot",
                    ),
                )
            ]
            ALV_PIC = "https://telegra.ph/file/6827fe35330c99e5a67c0.jpg"
            markup = event.client.build_reply_markup(buttons)
            photo = types.InputWebDocument(
            ...
            result = types.InputBotInlineResult(
                ...
                type="photo",
                title=f"†ê¢hñðßð†",
                description=f"†ê¢hñðßð†\nhttps://t.me/Technot_Support",
                url="https://github.com/TeamTechnot/Technot",
                thumb=photo,
                content=photo,
                send_message=types.InputBotInlineMessageMediaAuto(
                    reply_markup=markup, message=text, entities=msg_entities
...
@techno.tgbot.on(CallbackQuery(data=re.compile(b"clise")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    buttons = [
    ...
    await event.edit(
        f"📜 Menu Provider Has Been Closed\n\n🔰 Bot Of : {mention}\n\n             [©️†ê¢hñðßð†](https://t.me/Technot_Support)",
        buttons=buttons,
        link_preview=False,
    )


@techno.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@techno.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@techno.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
