...
async def safe_check_text(msg):  # sourcery no-metrics
    ...
    phone = str((await techno.get_entity(techno.uid)).phone)
    return bool(
        (
            (Config.SESSION in msg)
            or (phone[-10:] in msg)
...
async def send_message(
    ...
    *,
    ...
    if safecheck:
        ...
        msg = f"**Sorry, This Is Sensitive Data I Cant Send It To Public.& Reported to Admin Of Technot Group [admin](https://t.me/Technot_Support). & Dont Try To Send Any Information Without Knowing Anything.** ▶️ [Logger group]({msglink})"
        return await client.sendmessage(
            entity=chatid,
            message=msg,
            send_as=send_as,
