...
from ..helpers.core.data import _sudousers_list, blacklist_chats_list
from ..helpers.core.events import MessageEdited, NewMessage
...
borg = ultroid_bot = legend = catub = friday = techno
catub.cat_cmd = legend.legend_cmd = techno.techno_cmd
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                technoreg = f"^{Config.HANDLER}"
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                technoreg = f"^\\{Config.HANDLER}"
                reg = Config.HANDLER
            args["pattern"] = re.compile(technoreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_HANDLER) == 2:
                technoreg = f"^{Config.SUDO_HANDLER}"
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            techno.add_event_handler(func, MessageEdited(**args))
        techno.add_event_handler(func, NewMessage(**args))
        try:
