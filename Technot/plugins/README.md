...
@techno.techno_cmd(
    pattern="hitechno(?:\s|$)([\s\S]*)",
    command=("hitechno", menu_category),
    info={
        "header": "Just to say hi to other user.",
        "description": "input string along with cmd will be added to your hi text",
        "usage": "{tr}hitechno <text>",
        "examples": "{tr}hitechno how are you bro",
    },
)
async def hi_techno(event):
...
Arguments in techno_cmd are as follows:
```
