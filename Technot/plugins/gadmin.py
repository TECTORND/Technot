...
async def gpromote(technoevent):
    "To promote user in every group where you are admin(have a right to promote)."
    i = 0
    await technoevent.get_sender()
    me = await technoevent.client.get_me()
    techno = await eor(technoevent, "`Promoting globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await technoevent.get_chat()
    if technoevent.is_private:
        user = technoevent.chat
        rank = technoevent.pattern_match.group(1)
    else:
        technoevent.chat.title
    try:
        user, rank = await get_full_user(technoevent)
    except:
        pass
    if me == user:
        await techno.edit("You can't promote yourself...")
        return
    try:
        if not rank:
            rank = "Techno"
    except:
        return await techno.edit("**ERROR !!**")
    if user:
        telchanel = [
            d.entity.id
            for d in await technoevent.client.get_dialogs()
        ...
        for x in telchanel:
            try:
                await technoevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await techno.edit(f"**Promoting User in :**  `{i}` Chats...")
            except:
                pass
    else:
        await techno.edit(f"**Reply to a user !!**")
    await techno.edit(
        f"[{user.first_name}](tg://user?id={user.id}) **Was Promoted Globally In** `{i}` **Chats !!**"
    )
    await technoevent.client.send_message(
        BOTLOG_CHATID,
...
async def gdemote(technoevent):
    "To demote user in that group where you promote person to admin."
    i = 0
    await technoevent.get_sender()
    me = await technoevent.client.get_me()
    techno = await eor(technoevent, "`Demoting Globally...`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    if technoevent.is_private:
        user = technoevent.chat
    ...
    try:
        ...
        pass
    if me == user:
        await techno.edit("You can't Demote yourself !!")
        return
    try:
        if not rank:
            rank = "techno"
    ...
    if user:
        ...
        for x in telchanel:
            try:
                await technoevent.client(EditAdminRequest(x, user, rgt, rank))
                i += 1
                await techno.edit(f"**Demoting Globally In Chats :** `{i}`")
            except:
                pass
    else:
        await techno.edit(f"**Reply to a user !!**")
...
async def loban(event):
    ...
    techno = await eor(event, "`Ungban in progress...`")
    if event.is_private:
        user = await event.get_chat()
        reason = event.pattern_match.group(1)
...
@techno.techno_cmd(incoming=True)
