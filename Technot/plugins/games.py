...
async def truth_dare_task(event):
    ...
    if taskmode == "task":
        technoevent = await eor(event, "Getting a random task for you.....")
        taskmode = random.choice(["truth", "dare"])
    else:
        technoevent = await eor(event, f"Getting a random {taskmode} task for you.....")
    ...
    try:
        ...
        if taskmode == "truth":
            await technoevent.edit(f"**The truth task for you is**\n`{task}`")
        else:
            await technoevent.edit(f"**The dare task for you is**\n`{task}`")
    except Exception as e:
        await eod(technoevent, f"**Error while getting task**\n`{e}`", 7)
...
async def igame(event):
    ...
    if input_str not in game_code:
        technoevent = await eor(event, "`Give me a correct game code...`")
        await asyncio.sleep(1)
        await eod(
            technoevent, f"**Available Game Codes & Names :-**\n\n{game_list}", time=60
        )
    else:
        await eor(
            event,
