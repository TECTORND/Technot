...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        techno = await eor(var, "`Getting information...`")
        await asyncio.sleep(1.0)
        try:
            variable = var.pattern_match.group(2).split()[0]
            techno = "**ConfigVars**:" f"\n\n {variable} = `{heroku_var[variable]}`\n"
            if "SESSION" in variable:
                await eor(
                    var, "Techno String is a Sensetive Data.\nProtected By Technot"
                )
                return
            elif variable in heroku_var:
                await eor(var, techno)
            else:
        ...
        techno = await eor(var, "`Setting information...`")
        if not variable:
            return await techno.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if "SESSION" in variable:
            await techno.edit("Successfully Changed")
            return
        ...
        techno = await eor(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await techno.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await techno.edit(f"`{variable}`**  does not exist**")

        await techno.edit(f"`{variable}`  **successfully deleted**")
...
@techno.techno_cmd(
    ...
    info={
        "header": "To Check dyno usage of Technot and also to know how much left.",
        "usage": "{tr}usage",
    },
)
