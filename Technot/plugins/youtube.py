...
async def download_audio(event):
    ...
    technoevent = await eor(event, "`Preparing to download...`")
    ...
    for url in urls:
        try:
            ...
            vid_data = {"title": url, "uploader": "Technouserbot", "formats": []}
        startTime = time()
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await eod(technoevent, "__Unable to upload file__")
...
async def download_video(event):
    ...
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(technoevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("techno_ytv.mp4")
            print(f)
            technothumb = pathlib.Path("cat_ytv.jpg")
            if not os.path.exists(technothumb):
                technothumb = pathlib.Path("cat_ytv.webp")
            if not os.path.exists(technothumb):
                technothumb = None
            await technoevent.edit(
                f"`Preparing to upload video:`\
...
async def insta_dl(event):
    ...
    v2 = "@videomaniacbot"
    media_list = []
    technoevent = await eor(event, "**Downloading.....**")
    async with event.client.conversation(v1) as conv:
        try:
            v1_flag = await conv.send_message("/start")
        except YouBlockedUserError:
            await eor(
                technoevent, "**Error:** Trying to unblock & retry, wait a sec..."
            )
            await techno(unblock("instasave_bot"))
            v1_flag = await conv.send_message("/start")
        ...
        await eor(technoevent, "**Switching v2...**")
        async with event.client.conversation(v2) as conv:
            try:
                ...
                await techno(unblock("videomaniacbot"))
                v2_flag = await conv.send_message("/start")
