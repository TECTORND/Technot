...
from ..helpers.core import CMD_INFO, PLG_INFO
from ..helpers.core.data import _sudousers_list, sudo_enabled_cmds
...
@techno.techno_cmd(
    ...
    info={
        "header": "To enable or disable sudo of your Technouserbot.",
...
async def chat_blacklist(event):
    "To enable or disable sudo of your TechnoUserBot."
...
async def _(event):
    ...
    try:
        ...
        sudousers = {}
    if len(sudochats) == 0:
        return await eod(event, "__There are no sudo users for your Technouserbot.__")
    result = "**The list of sudo users for your TechnoUserbot are :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "-all":
        technoevent = await eor(event, "__Enabling all safe cmds for sudo....__")
        totalcmds = CMD_INFO.keys()
        typecmds = (
            PLG_INFO["botcontrols"]
            + PLG_INFO["autoprofile"]
        ...
        technoevent = await eor(event, "__Enabling compelete sudo for users....__")
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your TechnoUserBot__.\n"
                )
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += (
                    f"`{cmd}` __There is no such command in your TechnoUserBot__.\n"
                )
    ...
    for cmd in loadcmds:
        sqllist.add_to_list("sudo_enabled_cmds", cmd)
    result = (
        f"__Successfully enabled __ `{len(loadcmds)}` __ for TechnoUserBot sudo.__\n"
    )
    output = (
    ...
    if errors != "":
        output += "\n**Errors:**\n" + errors
    msg = await eor(technoevent, output)
    await event.client.reload(msg)
...
async def _(event):  # sourcery no-metrics
    ...
    input_str = input_str.split()
    if input_str[0] == "-all":
        technoevent = await eor(event, "__Disabling all enabled cmds for sudo....__")
        typecmds = sudocmds
    elif input_str[0] == "-type":
        technoevent = await eor(event, "__Disabling all typeged cmds for sudo.....__")
        typecmds = (
            PLG_INFO["botcontrols"]
            + PLG_INFO["autoprofile"]
            + PLG_INFO["evaluators"]
    ...
    result = f"__Successfully disabled __ `{count}` __ for TechnoUserBot sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
