# reverse search and google search  plugin for techno
import io
import os
import re
import urllib
...
@techno.techno_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}gs TechnoUserBot",
            "{tr}gs -l6 TechnoUserBot",
            "{tr}gs -p2 TechnoUserBot",
            "{tr}gs -p2 -l7 TechnoUserBot",
        ],
    },
)
async def gsearch(q_event):
    "Google search command."
    technoevent = await eor(q_event, "`searching........`")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        technoevent = await eor(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await technoevent.edit("Found Google Result. Pouring some soup on it!")
        headers = {
        ...
        try:
            ...
            return await eod(technoevent, "`Sorry. I am unable to find similar images`")
        end = datetime.now()
...
<i>fetched in {ms} seconds</i>""".format(
        ...
        technoevent = event
    await eor(technoevent, OUTPUT_STR, parse_mode="HTML", link_preview=False)
...
async def _(event):
    "To get info of pic."
    technoevent = await eor(event, "`processing........`")
    ...
    await technoevent.edit(f"`Downloaded to {downloaded_file_name}`")
    if downloaded_file_name.endswith((".webp")):
        resize_image(downloaded_file_name)
    try:
        media_urls = upload_file(downloaded_file_name)
    except exceptions.TelegraphException as exc:
        await technoevent.edit(f"**Error : **\n`{exc}`")
        os.remove(downloaded_file_name)
    else:
...
async def _(img):
    ...
    if message and message.media:
        ...
        return
    if photo:
        technoevent = await eor(img, "`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            return await technoevent.edit("`Unsupported , most likely.`")
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            ...
            return await technoevent.edit("`Unable to perform reverse search.`")
        ...
        if guess and imgspage:
            ...
            return await technoevent.edit("`Can't find any kind similar images.`")
