...
from ..helpers.utils import _format, _technotools, _technoutils, parse_pre, reply_id
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    if mediatype in ["Gif", "Video"]:
        return await eod(event, "Reply to supported Media...")
    technoevent = await eor(event, "__Making round spin video wait a sec.....__")
    output = await _technotools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
        return await eod(
    ...
    if output is None:
        return await eod(technoevent, "__Unable to make spin gif.__")
    ...
    TECHNO = await event.client.send_file(
        event.chat_id,
        media,
        reply_to=reply,
        video_note=True,
    ...
    if not args:
        await _technoutils.unsavegif(event, TECHNO)
...
async def video_swtfile(event):  # sourcery no-metrics
    ...
    type = True
    technoevent = await eor(event, "`Converting to round format..........`")
    ...
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not swtfile.endswith((".webp")):
            if swtfile.endswith((".tgs")):
                hmm = await make_gif(technoevent, swtfile)
        ...
        if swtthumb is not None and os.path.exists(swtthumb):
            ...
            os.remove(swtfile)
            return await eod(technoevent, "`No thumb found to make it video note`", 5)
    if mediatype in [
        "Voice",
        "Audio",
        "Gif",
        ...
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, technoevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
...
async def _(TECHNO):
    reply_to_id = TECHNO.message.id
    if TECHNO.reply_to_msg_id:
        reply_to_id = TECHNO.reply_to_msg_id
    event = await eor(TECHNO, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await TECHNO.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await TECHNO.client.send_file(
                TECHNO.chat_id,
...
async def png(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            await args.edit(f"`{random.choice(TECHNO)}`")
...
async def _(event):
    ...
    output = await _technotools.media_to_pic(event, reply)
    if output[1] is None:
        return await eod(
            output[0], "__Unable to extract image from the replied message.__"
        )
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    technoevent = await eor(event, "__🎞 Making Gif from the relied media...__")
    imag = await _technotools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await eod(
    ...
    try:
        ...
        return await eod(technoevent, f"**Error**\n__{e}__")
    output = io.BytesIO()
    output.name = "Output.gif"
    ...
    if output is None:
        ...
        for i in [final, "Output.gif", imag[1]]:
            if os.path.exists(i):
                os.remove(i)
        return
    TECHNO = await event.client.send_file(event.chat_id, output, reply_to=reply)
    await _technoutils.unsavegif(event, TECHNO)
    await technoevent.delete()
    for i in [final, "Output.gif", imag[1]]:
        if os.path.exists(i):
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    technoevent = await eor(event, "__🎞Converting into Gif..__")
    ...
    TECHNO = await event.client.send_file(event.chat_id, result, reply_to=reply)
    await _technoutils.unsavegif(event, TECHNO)
    await technoevent.delete()
