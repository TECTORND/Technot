...
async def grpinfo():
    outstr = "**Plugins in Technot are:**\n\n"
    outstr += f"**👨‍💻 Usage : ** `{cmdprefix}help <plugin name>`\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra", "useless"]
    for techno in category:
        plugins = GRP_INFO[techno]
        outstr += f"**{hemojis[techno]} {techno.title()} **({len(plugins)})\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your Technot are :**\n\n"
...
@techno.techno_cmd(
    ...
    info={
        "header": "To get guide for Technot.",
...
async def _(event):
    ...
    if not input_str:
        ...
        for cmd in cmds:
            outstr += f"  - `{cmdprefix}{cmd}`\n"
        outstr += f"**👨‍💻  Usage : ** `{cmdprefix}help -l <command name>`"
    await eor(event, outstr, aslink=True, linktext="Total Commands of Technot are :")
