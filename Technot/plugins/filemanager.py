...
@techno.techno_cmd(
    ...
    info={
        ...
        "usage": "{tr}ls <path>",
        "examples": "{tr}ls Technot",
    },
)
async def ls(event):  # sourcery no-metrics
    "To list all files and folders."
    techno = "".join(event.text.split(maxsplit=1)[1:])
    path = techno or os.getcwd()
    if not os.path.exists(path):
        await eor(
            event,
            f"there is no such directory or file with the name `{techno}` check again",
        )
        return
    path = Path(techno) if techno else os.getcwd()
...
async def rem(event):
    ...
    techno = event.pattern_match.group(1)
    if techno:
        path = Path(techno)
    else:
    ...
    if not os.path.exists(path):
        ...
        return
    technocmd = f"rm -rf '{path}'"
    if os.path.isdir(path):
        await _technoutils.runcmd(technocmd)
...
async def mkdir(event):
    ...
    try:
        await _technoutils.runcmd(f"mkdir {original}")
...
async def cpto(event):
    ...
    await asyncio.sleep(2)
    try:
        await _technoutils.runcmd(f"cp -r {original} {location}")
