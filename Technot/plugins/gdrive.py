...
# Technouserbot Google Drive managers  ported from Projectbish and added extra things by @TECHNO_K_BOY
...
async def download(event, gdrive, service, uri=None):  # sourcery no-metrics
    ...
    if uri:
        ...
        if swttorrent:
            ...
            await eor(
                gdrive,
                "`To use torrent files or download files from link install torrentutils from` @technoplugins",
            )
...
async def upload(gdrive, service, file_path, file_name, mimeType, dir_id=None):
    ...
    body = {
        "name": file_name,
        "description": "Uploaded from Telegram using Technouserbot.",
        "mimeType": mimeType,
...
async def g_download(event):
    ...
    technoevent = await eor(event, "`Downloading Requested File from G-Drive...`")
    file_name, swtprocess = await gdrive_download(
        event, technoevent, service, drive_link
    )
    if swtprocess is not None:
        return await eod(technoevent, file_name)
    ...
    if not cmd:
        ...
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, technoevent, c_time, "Uploading...", file_name)
            ),
        )
        os.remove(file_name)
        await eod(
...
async def gshare(event):
    ...
    technoevent = await eor(event, "`Creating sharable link...`")
    await asyncio.sleep(2)
    await share(service, technoevent, input_str)
