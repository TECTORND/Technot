...
async def note(e):
    ...
    img = Image.open("Technot/helpers/resources/extras/template.jpg")
    draw = ImageDraw.Draw(img)
    font = ImageFont.truetype("Technot/helpers/resources/fonts/assfont.ttf", 30)
    x, y = 150, 140
    lines = text_set(text)
...
async def write_page(event):
    ...
    if cmd == "notebook":
        text = (
            (await techno(GetFullUserRequest(techno.uid))).full_user
