...
from ..helpers.sql_helper import broadcast_sql as sql
...
@techno.techno_cmd(
    ...
    info={
        ...
        "examples": "{tr}msgto @Technot_Updates just a testmessage",
    },
)
async def technobroadcast_add(event):
...
async def technobroadcast_add(event):
    ...
    technoinput_str = event.pattern_match.group(1)
    if not technoinput_str:
        return await eod(
            event,
            "In Which category should i add this chat",
            parse_mode=_format.parse_pre,
        )
    keyword = technoinput_str.lower()
...
async def technobroadcast_list(event):
    ...
    await eor(technoevent, finaloutput)
...
async def technobroadcast_send(event):
    ...
    await eod(technoevent, resultext)
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
...
async def technobroadcast_remove(event):
...
async def technobroadcast_delete(event):
    "To delete a category completely."
    technoinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(technoinput_str)
    if check1 < 1:
        return await eod(
            event,
            f"Are you sure that there is category {technoinput_str}",
            parse_mode=_format.parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(technoinput_str)
        await eor(
            event,
            f"Successfully deleted the category {technoinput_str}",
            parse_mode=_format.parse_pre,
        )
    except Exception as e:
        await eod(
            ...
            try:
                if chat != -1001551357238:
                    await bot.send_message(
                        chat,
                        f"⣿⣿⣿⣿⣿⣍⠀⠉⠻⠟⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⡇⠀⠀⠀⠀⣰⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⣿⠓⠀⠀⢒⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⡿⠃⠀⠀⠀⠀⠈⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠿⣿\n⣿⡿⠋⠋⠀⠀⠀⠀⠀⠀⠈⠙⠻⢿⢿⣿⣿⡿⣿⣿⡟⠋⠀⢀⣩\n⣿⣿⡄⠀⠀⠀⠀⠀⠁⡀⠀⠀⠀⠀⠈⠉⠛⢷⣭⠉⠁⠀⠀⣿⣿\n⣇⣀. INDIA🇮🇳INDIA🇮🇳⠆⠠..⠘⢷⣿⣿⣛⠐⣶⣿⣿\n⣿⣄⠀⣰⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠐⢀⣠⣿⣿⣿⣾⣿⣿⣿\n⣿⣿⣿⣿⠀⠀⠀⠀⡠⠀⠀⠀⠀⠀⢀⣠⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⠀⠀⠀⠀⠀⠀⠀⠄⠀⣤⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⡄⠀⠀⠀⠀⠀⣠⣤⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⠀⠀⠂⠀⠀⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⣇⠀⠀⠀⢠⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⣿⡆⠀⢀⣼⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n⣿⣿⣿⣿⣿⣿⣿⣦⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿\n\n[нαρργ ιи∂ρєи∂єиϲє ∂αγ🇮🇳](https://t.me/Technot_Support)",
                    )
                    lol += 1
