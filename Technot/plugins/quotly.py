...
@techno.techno_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic TechnoUserBot.", "{tr}qpic -b TechnoUserBot."],
    },
)
async def q_pic(event):  # sourcery no-metrics
    ...
    technoevent = await eor(event, "__Making Quote pic....__")
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "TechnoUserBot.Webp"
        img.save(output, "webp")
    else:
        output.name = "TechnoUserBot.png"
        img.save(output, "PNG")
    output.seek(0)
...
@techno.techno_cmd(
    ...
    info={
        ...
        "examples": ["{tr}fq @Technoboy_02 hello bad boys and girls"],
    },
)
async def stickerchat(owoquotes):
    ...
    technoevent = await eor(owoquotes, "`Making quote...`")
    if cmd in ["rq", "q"]:
        try:
            user = (
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    technoevent = await eor(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if messages_id != []:
                ...
                return await eod(
                    technoevent, "`I guess you have used a invalid syntax`"
                )
