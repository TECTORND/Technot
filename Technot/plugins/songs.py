...
async def lyrics(event):  # sourcery no-metrics
    ...
    if "-" in query:
        ...
        artist = args[0].strip(" ")
        song = args[1].strip(" ")
        technoevent = await eor(event, f"`Searching lyrics for {artist} - {song}...`")
        try:
            songs = genius.search_song(song, artist)
        except TypeError:
            songs = None
        if songs is None:
            return await technoevent.edit(f"Song **{artist} - {song}** not found!")
        result = f"**Search query**: \n`{artist} - {song}`\n\n```{songs.lyrics}```"
    else:
        technoevent = await eor(event, f"`Searching lyrics for {query}...`")
...
async def spic(event):
    ...
    technoevent = await eor(event, "__Downloading the audio clip...__")
    try:
        ...
        LOGS.error(e)
        return await eod(
            technoevent, f"**Error while reverse searching song:**\n__{e}__"
        )
