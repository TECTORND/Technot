...
async def echo(event):
    "To start chatbot to the user messages by technoboy"
    if event.reply_to_msg_id is None:
        return await eor(event, "`Reply to a User's message to echo his messages`")
    technoevent = await eor(event, "`See Magic`")
    user, rank = await get_user_from_event(event, technoevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await eod(technoevent, f"**Error:**\n`{e}`")
    else:
        await eor(technoevent, "Hi")
...
async def sareply(event):
    if sis_echo(event.chat_id, event.sender_id) and (
        ...
        url = f"https://kukiapi.xyz/api/apikey=1356469075-KUKIkq4WMg5FV4/TechnoBoyAssistance/TechnoBoy/message={sweetie}"
