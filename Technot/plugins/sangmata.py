...
async def _(event):  # sourcery no-metrics
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(technoevent, "`unblock @Sangmatainfo_bot and then try`")
        responses = []
        while True:
            try:
    ...
    if not responses:
        await eod(technoevent, "`bot can't fetch results`")
    if "No records found" in responses:
        await eod(technoevent, "`The user doesn't have any record`")
    ...
    for i in check:
        if TECHNO:
            ...
            TECHNO = True
            await technoevent.edit(i, parse_mode=_format.parse_pre)
