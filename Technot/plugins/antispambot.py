...
# baning spmmers plugin for TechnoUserBot by @TECHNO_K_BOY
...
if Config.ANTISPAMBOT_BAN:
    ...
    @techno.on(ChatAction())
    async def anti_spambot(event):  # sourcery no-metrics
        if not event.user_joined and not event.user_added:
            return
        user = await event.get_user()
        technoadmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not technoadmin:
            return
        technobanned = None
        adder = None
        ...
        if is_gbanned(user.id):
            technogban = gbanned(user.id)
            if technogban.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{technogban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                LOGS.info(e)
        if spamwatch and not technobanned:
...
async def caschecker(event):
    ...
    try:
        ...
        await technoevent.edit("`CAS check failed: Admin privileges are required`")
        return
    except BaseException:
        await technoevent.edit("`CAS check failed`")
        return
    await technoevent.edit(text)
...
async def spamchecker(event):
    ...
    try:
        ...
        await technoevent.edit("`spamwatch check failed`")
        return
    await technoevent.edit(text)
