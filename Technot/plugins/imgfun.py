...
async def imirror(event):  # sourcery no-metrics
    ...
    technoevent = await event.edit("__Reflecting the image....__")
    args = event.pattern_match.group(1)
    if args:
        filename = "TechnoUserBot.webp"
        f_format = "webp"
    else:
        filename = "TechnoUserBot.jpg"
        f_format = "jpeg"
    try:
        imag = await _technotools.media_to_pic(technoevent, reply, noedits=True)
        if imag[1] is None:
            return await eod(
        ...
        return await eod(technoevent, f"**Error in identifying image:**\n__{e}__")
...
async def dc(event):
    ...
    r = requests.get(hmm)
    open("techno.gif", "wb").write(r.content)
    hehe = "techno.gif"
...
async def dc(event):
    ...
    r = requests.get(hmm)
    open("techno.png", "wb").write(r.content)
    hehe = "techno.png"
...
async def irotate(event):
    ...
    args = event.pattern_match.group(1)
    technoevent = await eor(event, "__Rotating the replied media...__")
    imag = await _technotools.media_to_pic(technoevent, reply, noedits=True)
    if imag[1] is None:
        return await eod(
            imag[0], "__Unable to extract image from the replied message.__"
...
async def iresize(event):
    ...
    technoevent = await eor(event, "__Resizeing the replied media...__")
    imag = await _technotools.media_to_pic(technoevent, reply, noedits=True)
    if imag[1] is None:
        return await eod(
            imag[0], "__Unable to extract image from the replied message.__"
    ...
    if len(args) == 1:
        try:
            ...
            return await eod(technoevent, "**Error:**\n__Invalid dimension.__")
    else:
        try:
            nw = int(args[0])
        except ValueError:
    ...
    try:
        ...
        return await eod(technoevent, f"**Error:** __While resizing.\n{e}__")
    await event.delete()
    img = io.BytesIO()
    img.name = "TechnoUserBot.png"
    image.save(img, "PNG")
...
async def square_cmd(event):
    ...
    technoevent = await event.edit("__Adding borders to make it square....__")
    try:
        imag = await _technotools.media_to_pic(technoevent, reply, noedits=True)
        if imag[1] is None:
            return await eod(
...
async def pic_gmd(event):
    ...
    if args:
        ...
        pix = 100
    technoevent = await eor(event, "__🎞Dotifying image...__")
    imag = await _technotools.media_to_pic(technoevent, reply, noedits=True)
    if imag[1] is None:
        return await eod(
            imag[0], "__Unable to extract image from the replied message.__"
