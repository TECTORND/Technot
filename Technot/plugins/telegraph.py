# telegraph utils for TechnoUserBot
import os
import random
import string
...
async def ctg(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await techno(unblock("chotamreaderbot"))
            msg_flag = await conv.send_message(urls[0])
        response = await conv.get_response()
        await event.client.send_read_acknowledge(conv.chat_id)
        if response.text.startswith(""):
            await eor(technoevent, "Am I Dumb Or Are U Dumb?")
        else:
...
async def _(event):
    ...
    if input_str in ["media", "m"]:
        ...
        ms = (end - start).seconds
        techno = f"https://telegra.ph/{response['path']}"
        await technoevent.edit(
            f"**✓ Uploaded to :-** [telegraph]({techno})\
                 \n**✓ Uploaded in {ms} seconds.**\
                 \n**✓ Uploaded by :-** {mention}\
                 \n**✓ Telegraph :-** `{techno}`",
            link_preview=True,
