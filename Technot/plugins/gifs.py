...
async def lol(event):
    ...
    async with event.client.conversation("@distortionerbot") as conv:
        ...
        if out in ["Gif", "Video", "Sticker"]:
            await _technoutils.unsavegif(event, owo)
...
async def some(event):
    ...
    technoevent = await eor(event, "`Sending gif....`")
    ...
    for items in rlist:
        ...
        await _technoutils.unsavegif(event, nood)
...
async def some(event):
    ...
    technoevent = await eor(event, "`Wait babe...`😘")
