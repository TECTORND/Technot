...
@techno.techno_cmd(
    ...
    info={
        ...
        "examples": "{tr}join @Technot_Official",
    },
)
async def lol(event):
    "Join Any Group"
    ...
    try:
        await techno(functions.channels.JoinChannelRequest(bol))
...
@techno.techno_cmd(
    ...
    info={
        ...
        "examples": "{tr}inviteall @technosgroupfortechnos",
        "note": "⚠️ If u using this cmd i am not responsible for ur id ban or delete",
    },
)
async def get_users(event):
    techn_ = event.text[10:]
    techno_chat = techn_.lower
    restricted = ["@Technot_Support", "@Technot_Updates"]
    TECHNO = await eor(event, f"**Inviting members from** {techn_}")
    if techno_chat in restricted:
        return await TECHNO.edit(event, "You can't Invite Members from there.")
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        await TECHNO.edit("`processing...`")
    else:
        await TECHNO.edit("`processing...`")
    if event.is_private:
        return await TECHNO.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"
    await TECHNO.edit(
        "**⚜️[Terminal Status](https://t.me/Technot_Support)**\n\n`👨‍💻Inviting Users.......`"
    )
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            ...
            tol = f"@{user.username}"
            lol = tol.split("`")
            await techno(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await TECHNO.edit(
...
async def get_users(event):
    ...
    f = 0
    error = "None"
    await TECHNO.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            if error.startswith("Too"):
                return await TECHNO.edit(
            ...
            tol = user.id
            await techno(InviteToChannelRequest(channel=event.chat_id, users=[tol]))
            s = s + 1
            await TECHNO.edit(
                f"**Terminal Running...**\n\n• Invited `{s}` people \n• Failed to Invite `{f}` people\n\n**× LastError:** `{error}`"
            )
