...
FF_MPEG_DOWN_LOAD_MEDIA_PATH = os.path.join(
    Config.TMP_DOWNLOAD_DIRECTORY, "TechnoUserBot.media.ffmpeg"
)
...
async def ffmpeg_compress(event):
    ...
    if not reply_message or not reply_message.media:
        if os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
            ...
            technoevent = await eod(event, "`Processing...`")
            delete = False
        else:
        ...
        if media not in ["Video", "Round Video", "Gif"]:
            return await eod(event, "`Only Video files are supported`")
        technoevent = await eor(event, "`Saving the file...`")
        try:
            c_time = time.time()
            ...
            await event.client.fast_download_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, technoevent, c_time, "Trying to download")
                ),
            )
            dl.close()
        except Exception as e:
            await eor(technoevent, f"**Error:**\n`{e}`")
        else:
            await eor(technoevent, "`Processing...`")
            delete = True
    else:
    ...
    compress = await convert_video(
        dlpath, "./temp", crf, old["duration"], techno, technoevent
    )
    cend = datetime.now()
    ...
    if compress:
        ...
        if cmd == "f":
            try:
                ...
                catt = await event.client.send_file(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, technoevent, c_time, "Trying to upload")
                    ),
                )
                os.remove(compress)
            except Exception as e:
                return await eod(technoevent, f"**Error : **`{e}`")
        else:
            thumb = await _technotools.take_screen_shot(compress, "00:01")
            try:
                c_time = time.time()
                catt = await event.client.send_file(
                    event.chat_id,
        ...
        return await eod(technoevent, "**ERROR :: Unalble to Compress**")
...
async def ff_mpeg_trim_cmd(event):
    ...
    reply_to_id = await reply_id(event)
    technoevent = await eor(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await eod(technoevent, "**Error : **`Can't complete the process`")
        try:
            c_time = time.time()
        ...
        cmd, start_time = cmt
        o = await _technotools.take_screen_shot(
        ...
        await eod(technoevent, "RTFM")
        return
    end = datetime.now()
    ms = (end - start).seconds
    await eod(technoevent, f"`Completed Process in {ms} seconds`", 3)
