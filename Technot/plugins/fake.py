...
async def _(event):
    ...
    user, rank = await get_user_from_event(event, technoevent)
    if not rank:
        rank = "Admin"
    if not user:
        return
    try:
        ...
        return await eod(technoevent, f"__{e}__", time=10)
    await technoevent.edit("`Promoted Successfully! Now gib Party`")
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 5122474448:
            await reply_message.reply(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 99$ to my master__ [TechnoBoy](https://t.me/Technoboy_02) __to release your account__😏"
            )
        else:
            jnl = (
                "`Warning!! `"
