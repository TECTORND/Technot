...
plugin for @Technot_Support
"""

from telethon.errors.rpcerrorlist import YouBlockedUserError

from . import eor, techno, reply_id
...
async def _(event):
    ...
    technoevent = await eor(event, "```Gathering info...```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await technoevent.edit("Unblock @cricbuzz_bot & try again")
            return
        if respond.text.startswith("I can't find that"):
            await technoevent.edit("sorry i can't find it")
        else:
