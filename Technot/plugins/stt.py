# speech to text module for TechnoUserBot by uniborg (@spechide)
import os
...
async def _(event):
    ...
    technoevent = await eor(event, "`Downloading to my local, for analysis  🙇`")
    required_file_name = await event.client.download_media(reply, Config.TEMP_DIR)
    await technoevent.edit("`Starting analysis, using IBM WatSon Speech To Text`")
    headers = {
    ...
    r = response.json()
    if "results" not in r:
        return await technoevent.edit(r["error"])
    ...
    await technoevent.edit(string_to_show)
