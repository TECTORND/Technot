...
async def _(event):
    ...
    technoevent = await eor(event, "`Downloading Profile Picture to my local ...`")
    ...
    try:
        ...
        if photo:
            await technoevent.edit("`now, Uploading to Telegram ...`")
            if photo.endswith((".mp4", ".MP4")):
                # https://t.me/tgbetachat/324694
                size = os.stat(photo).st_size
                if size > 2097152:
                    await technoevent.edit("`size must be less than 2 mb`")
                    os.remove(photo)
                    return
                lolpic = None
            ...
            try:
                ...
                await eor(technoevent, "`My profile picture was successfully changed`")
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.info(str(e))
